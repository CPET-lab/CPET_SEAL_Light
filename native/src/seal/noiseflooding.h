...
namespace seal
{
    // inline: this header is included from many translation units via seal.h --
    // without inline, each TU gets its own definition and the linker fails with
    // multiple-definition errors as soon as more than one gets linked together.
